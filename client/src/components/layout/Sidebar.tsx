...
import {
  Store, Globe, LayoutDashboard, PackageOpen, ShoppingCart,
  Users, LineChart, Settings, CreditCard, Brush, LogOut,
  FolderTree, BarChart2, Megaphone, Upload
} from "lucide-react";
...
const Sidebar = ({ collapsed = false }: SidebarProps) => {
  ...
  const navItems = isSuperAdmin
    ...
    : [
      ...
      { label: "Category Management", path: "/product-categories", icon: <FolderTree className="h-5 w-5" /> },
  ...
  return (
    <aside className={cn(
      ...
      <div className="p-4 flex justify-between items-center border-b h-16">
        <Link
          href="/"
          className={cn("flex items-center cursor-pointer hover:opacity-80 transition-opacity", collapsed && "justify-center w-full")}
        >
